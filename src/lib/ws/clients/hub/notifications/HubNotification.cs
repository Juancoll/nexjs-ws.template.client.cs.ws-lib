...
namespace nex.ws
{
    public class HubNotification<TUser, TToken>
    {
        #region [ fields ]   
        ...
        #region [ constructor ]
        public HubNotification(HubClient<TUser, TToken> hub, string service, string eventName)
        {
            _hub = hub;
            _service = service;
            _event = eventName;
        ...
        #region [ methods ]
        public HubNotification<TUser, TToken> On(Action action)
        {
            _actions.Add(action);
            return this;
        }
        public HubNotification<TUser, TToken> On(object group, Action action)
        {
            _actions.Add(action);
        ...
        public HubNotification<TUser, TToken> off()
        {
            _actions.Clear();
            _groups.Clear();
            return this;
        }
        public HubNotification<TUser, TToken> off(object group)
        {
            if (_groups.ContainsKey(group))
            {
        ...
        public HubNotification<TUser, TToken> off(Action action)
        {
